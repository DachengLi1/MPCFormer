...
def softmax(scores, dim):
    #print(scores[0][0][0])
    #hist = torch.histogram(scores.detach().cpu(), bins=100)
...
def softmax_pass(scores, attention_mask_zero_one, dim):
    # using c = 5 for now
    scores += 5
...
def softmax_pass2(scores, attention_mask_zero_one, dim):
    scores =  (scores + 5) ** 2
    scores *= attention_mask_zero_one
    scores = scores / torch.sum(scores, dim=dim, keepdims=True)
    return scores

def softmax_ln(scores, dim):
    a, b ,c, d = scores.size()
    ln = torch.nn.functional.layer_norm(scores, (b,c,d))
    ln = (ln + 1) / 2
    return ln / torch.sum(ln, dim=dim, keepdims=True)

def softmax_2quad(scores, dim):
    a, b ,c, d = scores.size()
    quad = torch.nn.functional.layer_norm(scores, (b,c,d))
    quad = quad**2 + 4*quad + 4
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_att=False):
        ...
        if self.softmax_type in ["pass", "pass2"]:
